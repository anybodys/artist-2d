...
class Action(Enum):
    ...
    def __new__(
        cls, action_type, action_callable, arg_transforms, output_transform=None
    ):
...
class TurtleAction(Action):
    ...
    CIRCLE = (
        ActionType.MOVE,
        turtle.circle,
        [_position, _angle, lambda x: int(x * 99) + 1],
    )
    ...
    PENCOLOR_R_READ = (
        ...
        turtle.pencolor,
        [],
        lambda c: _turtle_color_read(c, 0),
    )
    PENCOLOR_G_READ = (
        ActionType.READ,
        turtle.pencolor,
        [],
        lambda c: _turtle_color_read(c, 1),
    )
    PENCOLOR_B_READ = (
        ActionType.READ,
        turtle.pencolor,
        [],
        lambda c: _turtle_color_read(c, 2),
    )
    FILLCOLOR_R_READ = (
        ActionType.READ,
        turtle.fillcolor,
